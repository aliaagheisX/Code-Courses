...
DELIMITER &&
CREATE PROCEDURE activitesReport()
BEGIN
    SELECT 
...
DELIMITER $$
CREATE  PROCEDURE TopEnrolledCourses()
BEGIN
      SELECT 
...
DELIMITER &&
CREATE PROCEDURE TopLikedArticles()
BEGIN
    SELECT 
...
DELIMITER &&
CREATE PROCEDURE TopRatedCourses()
BEGIN
    SELECT 
    C.INSTRUCTORFNAME, C.INSTRUCTORSNAME, E.*, 
    (SELECT COUNT(L.SID) FROM ENROLL L WHERE L.CID = C.ID ) as enrolls_count,
...
DELIMITER &&
CREATE PROCEDURE TopTakenQuizzes()
BEGIN
...
DELIMITER &&
CREATE PROCEDURE getTopicsReport()
    READS SQL DATA
BEGIN
    SELECT T.NAME, T.ID ,
...
DELIMITER &&
CREATE PROCEDURE getUsersReport()
BEGIN
